...
for(i in 1:nrow(E_new)){
   tmp_E <- E_new[i,]
   tmp_pvnb <- pvnb(K = parameter$K, X0 = parameter$X0, p = parameter$p, c = parameter$c, C = C, r = parameter$r, q_matrix = q_matrix, E = tmp_E)
...
for(i in 1:nrow(q_new)){
   tmp_q <- q_new[i,]
   tmp_pvnb <- pvnb(K = parameter$K, X0 = parameter$X0, p = parameter$p, c = parameter$c, C = C, E = E, r = parameter$r, q_matrix = q_new)
   pvnb_vec <- append(pvnb_vec,tmp_pvnb)
}
...
for(i in 1:nrow(r_new)){
   tmp_r <- r_new[i,]
   tmp_pvnb <- pvnb(K = parameter$K, X0 = parameter$X0, p = parameter$p, c = parameter$c, C = C, E = E, r = tmp_r, q_matrix = q_matrix)
   pvnb_vec <- append(pvnb_vec,tmp_pvnb)
}
...
for(i in 1:nrow(k_new)){
   tmp_k <- k_new[i,]
   tmp_pvnb <- pvnb(X0 = parameter$X0, p = parameter$p, c = parameter$c, C = C, E = E, q_matrix = q_matrix, r = parameter$r, K = tmp_k)
   pvnb_vec <- append(pvnb_vec,tmp_pvnb)
}
...
for(i in 1:nrow(p_new)){
   tmp_p <- p_new[i,]
   tmp_pvnb <- pvnb(K = parameter$K, X0 = parameter$X0, r = parameter$r, c = parameter$c, C = C, E = E, q_matrix = q_matrix, p = tmp_p)
   pvnb_vec <- append(pvnb_vec,tmp_pvnb)
}
...
for(i in 1:nrow(C_new)){
   tmC_C <- C_new[i,]
   tmC_Cvnb <- pvnb(K = parameter$K, X0 = parameter$X0, p = parameter$p, c = parameter$c, r = parameter$r, E = E, q_matrix = q_matrix, C = tmC_C)
...
for(i in 1:nrow(x0_new)){
   tmx0_x0 <- x0_new[i,]
   tmx0_x0vnb <- pvnb(K = parameter$K, r = parameter$r, p = parameter$p, c = parameter$c, C = C, E = E, q_matrix = q_matrix, X0 = tmx0_x0)
...
#defaults
  ...
  rats <- as.data.frame(rbind(spec_1_rat, spec_2_rat, spec_3_rat, spec_4_rat))
  
  price_att1 = c(1,1,1,1)
  price_att2 = c(2,2,2,2)
    
  prices <- as.data.frame(rbind(price_att1, price_att2))
  
```  
run just varying possible ratios...
...
#sensitivity ranges
  ...
  spec_1_rat <- data.frame(spec = "spec_1", a1 = att_1, a2 = att_2) 
  spec_2_rat <- data.frame(spec = "spec_2", a1 = att_1,a2 = att_2)
  spec_3_rat <- data.frame(spec = "spec_3", a1 = att_1,a2 = att_2)
  spec_4_rat <- data.frame(spec = "spec_4", a1 = att_1,a2 = att_2)
...
for(i in 1:nrow(spec_1_rat)){
  for(j in 1:nrow(spec_2_rat)){
    for(k in 1:nrow(spec_3_rat)){
      for(l in 1:nrow(spec_4_rat)){
        ratio_new <- rbind(ratio_new, c(spec_1_rat$a1[i],spec_1_rat$a2[i],spec_2_rat$a1[j],spec_2_rat$a2[j], spec_3_rat$a1[k],spec_3_rat$a2[k],spec_4_rat$a1[l],spec_4_rat$a2[l]))
      }
    }
  }
}

ratio_new <- as.data.frame(ratio_new)

names(ratio_new) <- c("Sp1A1", "Sp1A2", "Sp2A1", "Sp2A2", "Sp3A1", "Sp3A2", "Sp4A1", "Sp4A2")
...
names(p_new) <- c("Sp1A1", "Sp1A2", "Sp2A1", "Sp2A2", "Sp3A1", "Sp3A2", "Sp4A1", "Sp4A2")

for(i in p_new[i,]){
  for(j in ratio_new[j,]){
    p_new[i,]*ratio_new[j,] #2 billion times! eek!
  }
}
...
p_new_t <- matrix(c(seq(1,16000,1)),
                    nrow=2000,
                    ncol=8)

for(i in 1:nrow(p_new_t)){
  test <- ratio_new*p_new_t[i,]
}
