...
qb_stock <- function(species, tech, cost, baskets, mortality, years){
  ...
  return(list(stock = stock_df, harvest = h_df, effort = e_df, profit = pft_df, profit_species = pft_sp_df))
}
...
results <- as.data.frame(test) %>% 
  mutate(stock_total=stock.1+stock.2,
         profit_total=profit.X1+profit.X2)
