...
qb_stock <- function(species, tech, cost, baskets, mortality, years){
  ...
  for (i in 1:years){
    ...
    viz_stock <- rowSums(stock_df[i,])
    b <- -viz_stock * mortality
    # Find optimal efforts
    E <- solve.QP(D,d,t_A,b, meq=1)$solution # Vector where the position is the effort per tech.
    ...
    stock_next <- stock_df[i,] + sp_df$r*stock_df[i,]*(1-stock_df[i,]/sp_df$K) - h
