...
source(here("working_model", "functions", "matrixCombn.R"))

source(here("working_model", "functions", "every_basket_all_res.R"))  
...
s1 <- c(s=1, r=0.15, K=params$species_k1, X=50, p=20) 
s2 <- c(s=2, r=0.2, K=params$species_k2, X=50, p=20)
s3 <- c(s=3, r=0.2, K=params$species_k3, X=50, p=20)
s4 <- c(s=4, r=0.3, K=params$species_k4, X=50, p=20)
s5 <- c(s=5, r=0.4, K=params$species_k5, X=50, p=20)
...
dfoutput <- dfoutput %>%
  mutate(tot_profit = t_1+t_2+t_3+t_4+t_5,
...
# stick all the NVPs, profits, and biomasses into a df, arrange from highest to lowest to find the optimal arrangement for each of these 3 criterion
