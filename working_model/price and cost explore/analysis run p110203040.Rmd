...
setwd("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/p1.10.20.30.40/")
...
tmp20n <- all_NPVs %>% 
  ...
  select(contains("s_1"):contains("s_5"), contains("t_1"):contains("t_2.3"), contains("tot_profit"), contains("tot_biomass"), contains("mortality")) %>% 
  mutate(run = "p20all") 
names(tmp20n) <- names_col
  
tmpn <- tmpn %>% 
  select(-contains("pall20"))

p_p110203040n <- tmpn %>% 
  select(contains("p1.10.20.30.40")) %>%
  mutate(run = "s1.10.20.30.40")
names(p_p110203040n) <- names_col

tidy_NPVs <- rbind(tmp20, p_p110203040)
...
p_p110203040p <- tmpp %>% 
  select(contains("p1.10.20.30.40")) %>%
  mutate(run = "s1.10.20.30.40")
names(p_p110203040p) <- names_col

tidy_av_pft <- rbind(tmp20p, p_p110203040p)
...
tmp20 <- all_av_biomass %>% 
  select(contains("pall20")) %>% 
  select(contains("s_1"):contains("s_5"), contains("t_1"):contains("t_2.3"), contains("tot_profit"), contains("tot_biomass"), contains("mortality")) %>% 
  mutate(run = "p20all") 
names(tmp20) <- names_col
...
p_p110203040 <- tmp %>% 
  select(contains("p1.10.20.30.40")) %>%
  mutate(run = "s1.10.20.30.40")
names(p_p110203040) <- names_col

tidy_av_biomass <- rbind(tmp20, p_p110203040)
...
run_factors <- factor(tidy_NPVs$run, levels = unique(tidy_NPVs$run),
                             labels = c("Ex: $20 all", "Ex: Sp1- $1, Sp2- $10, Sp3- $20, Sp4- $30, Sp5- $50"))
...
#NPV
pft_g_NPV <- ggplot()+
  geom_smooth(data = tidy_NPVs, aes(y=tot_pft, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
...
pft_g_av_pft <- ggplot()+
  geom_smooth(data = tidy_av_pft, aes(y=tot_pft, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
  labs(title= str_wrap("Profits for optimal-av-pft arrangements, ", 80),
         x= "Number of Years",
         y= "Fishery-wide Annual Profits",
       color= "Prices")+
  scale_y_continuous(labels=dollar_format(prefix="$"))+
...
#biomass_av
pft_g_av_biomass <- ggplot()+
  geom_smooth(data = tidy_av_biomass, aes(y=tot_pft, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
  labs(title= str_wrap("Profits for optimal-av_biomass arrangements, ", 80),
         x= "Number of Years",
         y= "Fishery-wide Annual Profits",
       color= "Prices")+
  scale_y_continuous(labels=dollar_format(prefix="$"))+
...
stock_g_NPV <- ggplot()+
  geom_smooth(data = tidy_NPVs, aes(y=tot_biomass, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
...
stock_g_av_pft <- ggplot()+
  geom_smooth(data = tidy_av_pft, aes(y=tot_biomass, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
  labs(title= str_wrap("Stock Level for optimal-NPV arrangements, ", 80),
         x= "Number of Years",
         y= "Fishery-wide Annual Stock Levels",
       color= "Prices")+
...
stock_g_av_biomass <- ggplot()+
  geom_smooth(data = tidy_av_biomass, aes(y=tot_biomass, x=rep(0:30,2), color = as.factor(run)), se = FALSE)+ theme_minimal()+
  labs(title= str_wrap("Stock Level for optimal-NPV arrangements, ", 80),
         x= "Number of Years",
         y= "Fishery-wide Annual Stock Levels",
       color= "Prices")+
...
comparez <- rbind(filter(tidy_av_biomass2, run == "Ex: Sp1- $1, Sp2- $10, Sp3- $20, Sp4- $30, Sp5- $50"),
                        filter(tidy_av_pft2, run == "Ex: Sp1- $1, Sp2- $10, Sp3- $20, Sp4- $30, Sp5- $50"),
                        filter(tidy_NPVs2, run == "Ex: Sp1- $1, Sp2- $10, Sp3- $20, Sp4- $30, Sp5- $50"),
...
profit_comp <- ggplot()+
  geom_smooth(data = comparez, aes(y=tot_pft, x=rep(0:30,4), color = as.factor(optimal)), se = FALSE, na.rm = TRUE)+ theme_minimal()+
