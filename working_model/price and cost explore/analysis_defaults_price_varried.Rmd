...
output_20 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreoutput_b_p20.csv")
...
output_p1t40 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreoutput_b_p1t40.csv")
NPV_p1t40 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreNPV_b_p1t40.csv")

#output_p1all40 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreoutput_b_p1all40.csv")
#NPV_p1all40 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreNPV_b_p1all40.csv")

# got error when ran p1all40

output_p10.70t100 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreoutput_b_p10.70t100.csv")
NPV_p10.70t100 <- read_csv("C:/Users/kll86/OneDrive/Documents/UCSB/quotabasket_model/working_model/price and cost explore/model run result files/price and cost exploreNPV_b_p10.70t100.csv")


NPVs <- rbind(NPV_20, #NPV_p1all40,
              NPV_p1t40, NPV_p10.70t100)
...
# pfts_p1all40 <- output_p1all40%>%    select(profit_per_t.t_1:profit_per_t.t_5) %>% 
#   mutate(run = "1,1,1,1,40")

pfts_p1t40 <- output_p1t40%>%    select(profit_per_t.t_1:profit_per_t.t_5) %>% 
  mutate(run = "1,10,20,30,40")

pfts_p10.70t100 <- output_p10.70t100%>%    select(profit_per_t.t_1:profit_per_t.t_5) %>% 
  mutate(run = "10,70,80,90,100")

combo_pfts <- rbind(pfts_20, #pfts_p1all40,
                    pfts_p1t40, pfts_p10.70t100)
...
combo_stock <- rbind(stock_20, #stock_p1all40,
                     stock_p1t40, stock_p10.70t100)
...
```{r}
stock_g <- ggplot()+
  geom_smooth(data = combo_stock_f, aes(y=tots, x=rep(0:30,3), color = as.factor(run)), se = FALSE)+ theme_minimal()+
...
longer_stock_f$run <- factor(longer_stock_f$run, levels = unique(longer_stock_f$run),
                             labels = c("Ex: Uniform Prices", #"Ex: Single High Price",
                                        "Ex: Ranging Prices", "Ex: Low and High Prices"))
...
stock_comp_g <- grid.arrange(p1,p3,p4, ncol=2,
  top=textGrob(str_wrap("Predicted annual stock levels for a fishery with varried prices under 2 quota baskets", 100), gp=gpar(fontsize = 13)),
       bottom= textGrob(str_wrap("Results of a 2 quota basket model for 5 species and 5 technologies parameterized such that species are identical in the following parameters: initial stock size = 50, fishing costs = 1, carrying capacities = 100, coefficient for quota basket limits = 0.2. The coefficient for quota basket limit defines the total combined biomass that can be extracted per year as a proportion of the total available biomass for that year. Species differ in terms of: their respective market prices, growth rates = 0.15, 0.2, 0.2, 0.3, 0.4 (respective to each species, 1-5), catchabilities per technology = all 0.01 except for t1 catches species1 at 0.04, t2 catches species2 at 0.04, t3 catches species3 at 0.04, t4 catches species4 at 0.04, and t5 catches species5 at 0.04. Model was run for 3 different price arrangements (see legend). Note: an arrangement of 4 species with prices of $1 and 1 species of price $40 was attempted, but due to an error was omitted.", 140), gp=gpar(fontsize = 8)),
