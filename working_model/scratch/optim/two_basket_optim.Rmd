...
every_basket <- function(species, tech, cost, n_baskets, mort_guess, years){
  ...
  bmtx <- matrixCombn(n_baskets, length(species))
  ...
  basket_fx <- function(basket_list){
        ...
        end_biomass = tail(result$species, 1)
        return(list(profit, end_biomass)
  }
  plan(multiprocess, workers = 5)
  result <- future.apply::future_lapply(bmtx, possibly(basket_fx, otherwise = NA))
  
  profit <- tibble(unlist(result$profit))
  end_biomass <- as_tibble(split(result$end_biomass, 1:nrow(end_biomass)))
  return(list(profits = profit, biomass <- end_biomass)
}

every_basket(species, tech, cost, n_baskets = 2, mort_guess, years)
