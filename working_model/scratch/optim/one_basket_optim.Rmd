...
biomass <- tibble(tail(output$stock, 1))

profits <- tibble(sum(colSums(output$profit_per_t, na.rm = TRUE)))

write_csv(profits, here("working_model", "scratch", "optim", "data", "one_basket_profits.csv"))
write_csv(biomass, here("working_model", "scratch", "optim", "data", "one_basket_biomass.csv"))
```