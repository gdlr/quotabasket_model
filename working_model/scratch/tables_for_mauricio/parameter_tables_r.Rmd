---
title: "parameter_tables_mauricio_r"
...
s1 <- c(s=1, r=0.2, K=100, X=50, p=20) #low r
s2 <- c(s=2, r=0.2, K=100, X=50, p=20) #low r
s3 <- c(s=3, r=0.2, K=100, X=50, p=20) #low r
s4 <- c(s=4, r=0.4, K=100, X=50, p=20) #high r
s5 <- c(s=5, r=0.4, K=100, X=50, p=20) #high r
...
kbl(species_df, escape = F, caption = "Species Parameters for baskets based on r", col.names = c("Species", "r", "K", "Starting stock", "Price", "Run 1", "Run 2", "Run 3")) %>% 
   ...
   save_kable(file = here("working_model", "scratch", "tables_for_mauricio", "r_sp_params.png"), zoom = 2)


kbl(tech_df, caption = "Technology matrix for baskets based on price", col.names = c("Technology", "Catchability\n sp. 1", "Catchability\n sp. 2", "Catchability\n sp. 3", "Catchability\n sp. 4", "Catchability\n sp. 5"), escape = F) %>% 
  kable_classic(full_width = F)  %>% 
  save_kable(file = here("working_model", "scratch", "tables_for_mauricio", "r_t_params.png"), zoom = 2)
```
