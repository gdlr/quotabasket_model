...
qb_max_p <- function(baskets, species, tech, cost, years){
  mort <- seq(0.1, 0.5, by = 0.05)
  mort_full = expand.grid(replicate(length(baskets),mort, F))
  p_out <- data_frame(m1 = 0, m2 = 0, m3 = 0, m4 = 0, m5 = 0, prof = 0)
  
  mort_list <- as.list(data.frame(t(mort_full)))
  
  fx <- function(mort_list){
    tmp_m <- unlist(mort_list)
    out <- qb_stock(species, tech, cost, baskets, tmp_m, years)
    total_prof <- sum(colSums(out$profit_per_t, na.rm = TRUE))
    vec <- c(mort_list, total_prof)
    return(vec)
  }
  
  plan(multiprocess, workers = 5)
  
  future.apply::future_lapply(mort_list, fx)
}
...
max_p_df <- do.call(rbind.data.frame, max_p)

colnames(max_p_df) <- c("m1", "m2", "m3", "m4", "m5", "profit")

max_row <- which.max(max_p_df$profit)

max_prof <- max_p_df[max_row,]

mort <- c(0.15, 0.15, 0.1, 0.15, 0.25)


output <- qb_stock(species, tech, cost, baskets, mort, years)
