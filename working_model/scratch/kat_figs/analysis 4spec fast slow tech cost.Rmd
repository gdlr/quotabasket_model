...
#setwd("./working_model/price and cost explore/model run result files/4spec/costs/def/cdef1122/")
temp = list.files(here("working_model", "price and cost explore", "model run result files", "4spec", "costs", "def", "cdef1122"), pattern=".csv")
for (i in 1:length(temp))
  {assign(paste("runz", temp[i], sep = "."), read_csv(here("working_model", "price and cost explore", temp[i])))}

```

```{r}
# get specific data for run ident spec 2211
temp = list.files(here("working_model", "price and cost explore", "model run result files", "4spec", "costs", "ident", "cident1122"), pattern=".csv")
for (i in 1:length(temp))
  {assign(paste("runz", temp[i], sep = "."), read_csv(here("working_model", "price and cost explore",temp[i])))}
...
biomass_plot <- plot %>% 
  mutate(year = rep(0:30, nrow(plot)/30)) %>% 
  select(1:4, run, year) %>% 
  pivot_longer(1:4, 
               values_to = "biomass",
               names_to = "species") %>% 
  mutate(value = case_when(
    run %in% "2 slow-growing with high costs" & species %in% "s_1" ~ "high",
    run %in% "2 slow-growing with high costs" & species %in% "s_3" ~ "high",
    run %in% "2 slow-growing with high costs" & species %in% "s_4" ~ "low",
    run %in% "2 slow-growing with high costs" & species %in% "s_5" ~ "low",
    
    run %in% "2 slow-growing with low costs" & species %in% "s_1" ~ "low",
    run %in% "2 slow-growing with low costs" & species %in% "s_3" ~ "low",
    run %in% "2 slow-growing with low costs" & species %in% "s_4" ~ "high",
    run %in% "2 slow-growing with low costs" & species %in% "s_5" ~ "high",
    
    run %in% "sp1: high, sp3: low, sp4: high, sp5: low" & species %in% "s_1" ~ "high",
    run %in% "sp1: high, sp3: low, sp4: high, sp5: low" & species %in% "s_3" ~ "low",
    run %in% "sp1: high, sp3: low, sp4: high, sp5: low" & species %in% "s_4" ~ "high",
    run %in% "sp1: high, sp3: low, sp4: high, sp5: low" & species %in% "s_5" ~ "low",
    
    run %in% "sp1: low, sp3: high, sp4: low, sp5: high" & species %in% "s_1" ~ "low",
    run %in% "sp1: low, sp3: high, sp4: low, sp5: high" & species %in% "s_3" ~ "high",
    run %in% "sp1: low, sp3: high, sp4: low, sp5: high" & species %in% "s_4" ~ "low",
    run %in% "sp1: low, sp3: high, sp4: low, sp5: high" & species %in% "s_5" ~ "high",
    
  ))

cost_plot <- ggplot(data = biomass_plot) +
  geom_line(aes(y = biomass, x = year, color = value, linetype = species), size = 0.8) +
  facet_wrap(~run) +
  theme_bw() +
...
ggsave(plot = cost_plot, here("working_model", "scratch", "kat_figs", "figs", "cost_default_params.png"))
...
```{r}
new_pft <- ggplot()+
  geom_line(data = plot, aes(y=tot_pft, x=rep(seq(0, 30),nrow(plot)/30), 
...
# These are the high price species:
...
stock_comp_g <- (p7|p8|p9|p10) / (p3|p4|p5|p6) + plot_annotation(
