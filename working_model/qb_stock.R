qb_stock <- function(species, tech, cost, baskets, mortality, years){
  ...
  for (i in 1:years){
    ...
    # d combines the P,B,and Z matrices yeilding the per tech revenue (1xtech)
  ...
  return(list(stock = stock_df, harvest = h_df, effort = e_df, profit = pft_df, profit_species = pft_sp_df,years=horizon))
}