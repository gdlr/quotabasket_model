...
#sensitivity ranges
  ...
  spec_1_rat <- data.frame(a1 = att_1,a2 = att_2) 
  spec_2_rat <- data.frame(a1 = att_1,a2 = att_2)
  spec_3_rat <- data.frame(a1 = att_1,a2 = att_2)
  spec_4_rat <- data.frame(a1 = att_1,a2 = att_2)
  
  attribute_2_to_species_ratios <- as.data.frame(cbind(spec_1_rat, spec_2_rat, spec_3_rat, spec_4_rat))
```
...
for(i in 1:nrow(rat_new)){
   tmp_rat <- rat_new[i,]
   tmp_pvnb <- pvnb(K = parameter$K, X0 = parameter$X0, p = parameter$p, c = parameter$c, C = C, E = E, r = parameter$r, q_matrix = q_matrix, tmp_rat)
   pvnb_vec <- append(pvnb_vec,tmp_pvnb)
}
